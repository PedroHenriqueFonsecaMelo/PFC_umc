...
class LivroAdminServiceTest {
    ...
    void deveListarLivrosAprovadosPaginado_SemBusca() {
        ...
        Page<Livro> pageMock = new PageImpl<>(List.of(Livro.builder().titulo("A").build()));

        when(livroRepository.findByAprovadoTrue(pageable)).thenReturn(pageMock);

        Page<Livro> result = service.listarLivrosAprovadosPaginado(pageable, null);

        assertEquals(1, result.getTotalElements());
        verify(livroRepository).findByAprovadoTrue(pageable);
    }
    ...
    void deveListarLivrosAprovadosPaginado_ComBusca() {
        ...
        Page<Livro> result = service.listarLivrosAprovadosPaginado(pageable, "java");
    ...
    void deveListarPromocoesAtivas_SemBusca() {
        ...
        when(livroRepository.findPromocoesAtivasPaginado(any(), eq(pageable)))
                .thenReturn(page);

        Page<Livro> result = service.listarPromocoesAtivasPaginado(pageable, null);
    ...
    void deveListarPromocoesAtivas_ComBusca() {
        ...
        when(livroRepository.findPromocoesAtivasPaginadoComBusca(
                any(), anyString(), anyString(), eq(pageable)))
                .thenReturn(page);

        Page<Livro> result = service.listarPromocoesAtivasPaginado(pageable, "teste");
