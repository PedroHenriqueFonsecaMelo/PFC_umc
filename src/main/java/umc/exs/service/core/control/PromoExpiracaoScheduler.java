 ...
 * Roda a cada hora; para cada livro com promocaoExpira < now():
 * - restaura precoAprovado para precoOriginal
 * - limpa os campos de promoção
 */
@Slf4j
@Component
...
public class PromoExpiracaoScheduler {
    ...
    public void expirarPromocoes() {
        ...
        for (Livro livro : expirados) {
            Double precoAnterior = livro.getPrecoOriginal();
            livro.setEmPromocao(false);
            livro.setPrecoAprovado(precoAnterior != null ? precoAnterior : livro.getPrecoAprovado());
            livro.setPrecoOriginal(null);
            livro.setPromocaoExpira(null);
            livroRepository.save(livro);
            log.info("Promoção expirada: livro ID {} — \"{}\"", livro.getId(), livro.getTitulo());
        }
