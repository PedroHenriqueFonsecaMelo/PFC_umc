...
function totalCarrinho() {
    return getCarrinho().reduce(
        (sum, item) => sum + (item.precoAprovado || 0),
        0,
    );
}
...
function atualizarBotoes() {
    ...
    document.querySelectorAll(".btn-carrinho").forEach((btn) => {
        ...
        if (ids.has(id)) {
            ...
            btn.innerHTML =
                '<img src="/imagens/estante.png" style="width:48px;height:48px;object-fit:contain;vertical-align:middle;margin-right:5px"> Colocar na estante';
...
function mostrarToastResultado(data) {
    ...
    if (ok > 0 && fail === 0) {
        ...
        toast.innerHTML = `✅ ${ok} livro(s) comprado(s)! Saldo restante: T$ ${
            data.saldoRestante.toFixed(2)
        }`;
    } else if (ok > 0 && fail > 0) {
        toast.className = "toast toast-aviso";
        toast.innerHTML = `⚠️ ${ok} comprado(s), ${fail} falhou. Saldo: T$ ${
            data.saldoRestante.toFixed(2)
        }`;
    } else {
...
async function carregarSaldo() {
    try {
        ...
        if (res.ok) {
            ...
            if (el) {
                el.innerHTML =
                    `<i class="fa-solid fa-coins mr-1 text-yellow-400"></i> T$ ${
                        c.saldoTokens.toFixed(2)
                    }`;
            }
        }
    } catch (_) {}
...
async function carregarLivros() {
    ...
    try {
        ...
        if (!livros.length) {
            grid.innerHTML =
                `<p style="grid-column:1/-1;text-align:center;color:#7A6E65;padding:2.5rem 0;font-family:'IM Fell English',serif;font-style:italic">Nenhum livro disponível ainda.</p>`;
            return;
        }
        ...
        console.error(err);
        grid.innerHTML =
            `<p style="grid-column:1/-1;text-align:center;color:#722F37;padding:2.5rem 0;font-family:'IM Fell English',serif;font-style:italic">Erro ao carregar vitrine.</p>`;
    }
}
...
function handleAdicionarCarrinho(btn, livro) {
    ...
    if (toast) {
        ...
        toast.innerHTML =
            `<img src="/imagens/estante.png" style="width:48px;height:48px;object-fit:contain;vertical-align:middle;margin-right:5px"> <strong>${livro.titulo}</strong> adicionado à estante`;
        toast.style.display = "block";
        setTimeout(() => {
            toast.style.display = "none";
        }, 2500);
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    carregarLivros();
    renderCarrinho();
    document.getElementById("carrinhoOverlay")?.addEventListener(
        "click",
        fecharCarrinho,
    );
});
