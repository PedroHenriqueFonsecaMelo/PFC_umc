...
function avaliarForcaSenha(senha) {
    ...
    if (senha.length < 8 || SENHAS_PROIBIDAS.has(senha.toLowerCase())) {
        return "fraca";
    }
    ...
    const criterios =
        [temMaiuscula, temMinuscula, temNumero, temEspecial].filter(Boolean)
            .length;
    const temSequencia =
        /012|123|234|345|456|567|678|789|890|abc|bcd|cde|def|efg|fgh|ghi|hij|ijk|jkl|klm|lmn|mno|nop|opq|pqr|qrs|rst|stu|tuv|uvw|vwx|wxy|xyz|aaa|bbb|ccc|ddd|111|222|333|444|555|666|777|888|999/i
            .test(senha);
...
function initForcaSenha(campoId, containerId) {
    ...
    campo.addEventListener("input", () => {
        ...
        const c = cfg[forca];
        segs.forEach((seg, i) => {
            seg.style.background = i < c.ativo ? c.cor : "#e0e0e0";
        });
        if (label) {
            label.textContent = c.texto;
...
function gerarEPreencher(senhaId, confirmarId, btnCopiarId) {
    ...
    if (btnCopiar) {
        ...
        const wrap = btnCopiar.closest
            ? btnCopiar.closest(".copiar-container")
            : btnCopiar.parentElement;
...
function initConfirmarSenha() {
    ...
    function podeSubmeter() {
        ...
        btn.disabled =
            !(nomeOk && emailOk && dataOk && cpfOk && senhaValida && coincide &&
                termsOk && privacyOk);
    }
...
function initResetSenha() {
    ...
    function podeSubmeter() {
        ...
        const coincide = confirmar.value.length > 0 &&
...
function initAlterarSenha() {
    ...
    if (!senhaAtual || !novaSenha || !confirmarSenha || !msgConfirmar || !btn) {
        return;
    }
    ...
    function atualizarBtn() {
        ...
        const forcaOk = forca !== null && forca !== "fraca";
        const confirmacaoOk = confirmarSenha.value.length > 0 &&
            confirmarSenha.value === novaSenha.value;
        const novaIgualAtual = novaSenha.value.length > 0 &&
            senhaAtual.value.length > 0 && novaSenha.value === senhaAtual.value;
    ...
    function validarNovaIgualAtual() {
        if (msgIgualAtual) {
            const igual = novaSenha.value.length > 0 &&
...
function initDynamicForms() {
    ...
    if (btnAddEndereco) {
        btnAddEndereco.addEventListener("click", () =>
            adicionarItem("enderecos"));
    }
    if (btnAddCartao) {
        btnAddCartao.addEventListener("click", () =>
            adicionarItem("cartoes"));
    }
}
...
function adicionarItem(tipo) {
    const containerId = tipo === "enderecos"
        ? "enderecos-list-container"
