...
function corMedalha(pos) {
    return ["var(--gold)", "var(--silver)", "var(--bronze)"][pos - 1] ||
        "var(--accent)";
}
...
async function detectarLogin() {
    try {
        ...
        if (_minhaPosicao > 0) {
            ...
            if (badge) {
                badge.textContent = `Você está em ${_minhaPosicao}º lugar`;
            }
...
async function carregarRanking(periodo) {
    ...
    document.getElementById("rkTabela").innerHTML =
        '<div class="rk-loading">Carregando ranking…</div>';

    try {
        const res = await fetch(
            `/api/ranking/top?limite=100&periodo=${_periodoAtual}`,
        );
...
function renderPodio(top3) {
    ...
    if (!top3 || top3.length === 0) {
        container.innerHTML =
            '<p class="rk-vazio">Nenhum leitor no ranking ainda.</p>';
        return;
    }
    ...
    container.innerHTML = top3.map((u) => {
        ...
        const pct = u.xpTotal > 0 && u.xpProximoNivel >= 0
            ? Math.min(
                100,
                Math.round(
                    (u.xpTotal / (u.xpTotal + (u.xpProximoNivel || 1))) * 100,
                ),
            )
            : 100;
        const avatarHtml = u.fotoPerfil
            ? `<img src="${
                esc(u.fotoPerfil)
            }" alt="${nomeCurto}" class="rk-card-avatar-img" style="border-color:${
                corMedalha(u.posicao)
            }">`
            : `<div class="rk-card-avatar" style="border-color:${
                corMedalha(u.posicao)
            }">${inicial(u.primeiroNome)}</div>`;
        ...
        <div class="rk-card pos-${u.posicao}${eVoce ? " e-voce" : ""}">
            ...
            <div class="rk-card-xp-num">${
            u.xpTotal.toLocaleString("pt-BR")
        }</div>
...
function renderTabela(items) {
    ...
    if (!items || items.length === 0) {
        container.innerHTML =
            '<p class="rk-vazio" style="padding:1.5rem 0">Apenas top 3 no ranking.</p>';
        return;
    }
    ...
    container.innerHTML = items.map((u) => {
        ...
        const avatarLinha = u.fotoPerfil
            ? `<img src="${
                esc(u.fotoPerfil)
            }" alt="${nomeCurto}" class="rk-linha-avatar-img">`
        ...
        <div class="rk-linha${eVoce ? " e-voce" : ""}">
            ...
            <div class="rk-linha-info">
                ...
                <div class="rk-linha-nivel">${esc(u.badge)} ${
            esc(u.nivel)
        }</div>
            </div>
...
window.rkFiltrar = function (periodo, btn) {
    document.querySelectorAll(".rk-filtro-btn").forEach((b) =>
