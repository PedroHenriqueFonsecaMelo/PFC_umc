...
document.addEventListener("DOMContentLoaded", () => {
    ...
    let dados = null;
    try {
        dados = JSON.parse(sessionStorage.getItem(CONF_KEY));
    } catch (_) {}
    ...
    if (
        !dados || !Array.isArray(dados.comprados) ||
    ...
    // Cards dos pedidos
    document.getElementById("listaPedidos").innerHTML = dados.comprados.map(
        (item, idx) => {
            const delay = idx * 80;
        ...
        <div class="pedido-card" style="animation-delay:${delay}ms">
          <div class="pedido-card-topo">
            <div class="pedido-id-wrap">
              ...
              <strong class="pedido-id">${
                item.codigoPedido || ("#" + (item.pedidoId || "—"))
            }</strong>
            </div>
    ...
    document.getElementById("resumoTotal").textContent = `T$ ${
        (dados.totalGasto || 0).toFixed(2)
    }`;
    document.getElementById("resumoSaldo").textContent = `T$ ${
        (dados.saldoRestante || 0).toFixed(2)
    }`;
});
