...
function renderBookCover(livro) {
    // Lombo
    document.getElementById("bookSpineText").textContent = livro.titulo +
        " · " + livro.autor;
    ...
    document.getElementById("bookBackText").textContent = resumo.length > 400
        ? resumo.slice(0, 400) + "…"
        : resumo;
    document.getElementById("bookBackIsbn").textContent = livro.isbn
        ? "ISBN " + livro.isbn
        : "";
...
window.trocarFoto = function (url, thumb) {
    ...
    document.querySelectorAll(".galeria-thumb").forEach((t) =>
...
window.navegarLightbox = function (dir) {
    _lightboxIndice = (_lightboxIndice + dir + _lightboxFotos.length) %
        _lightboxFotos.length;
    const img = document.getElementById("lightboxImg");
...
function exibirMetadados(livro, gbData) {
    ...
    if (gbData.dataPublicacao) {
        itens.push({
            icone: "📅",
            label: "Publicação",
            valor: formatarDataPublicacao(gbData.dataPublicacao),
        });
    }
    if (gbData.idioma) {
        const idiomas = {
        ...
        itens.push({
            icone: "🌐",
            label: "Idioma",
            valor: idiomas[gbData.idioma] || gbData.idioma,
        });
    }
...
function renderLivro(livro) {
    ...
    document.getElementById("livroAutor").textContent = `por ${
        livro.autor || "—"
    }`;
    document.getElementById("livroIsbn").textContent = `ISBN: ${
        livro.isbn || "—"
    }`;
