...
function randomColor(opts) {
  ...
  if (opts && opts.notColor) {
    while (color === opts.notColor) color = COLOR_CODES[Math.random()*COLOR_CODES.length|0];
  }
  return color;
}
...
const Star = {
  ...
  add(x,y,color,angle,speed,life,sox,soy){
    ...
    i.visible=true;i.heavy=false;i.x=x;i.y=y;i.prevX=x;i.prevY=y;i.color=color;
    i.speedX=Math.sin(angle)*speed+(sox||0);
    i.speedY=Math.cos(angle)*speed+(soy||0);
    i.life=life;i.fullLife=life;
    i.spinAngle=Math.random()*PI_2;i.spinSpeed=0.8;i.spinRadius=0;
    i.sparkFreq=0;i.sparkSpeed=1;i.sparkTimer=0;
    i.sparkColor=color;i.sparkLife=750;i.sparkLifeVariation=0.25;
    i.strobe=false;i.updateFrame=0;i.onDeath=null;i.secondColor=null;
    i.transitionTime=0;i.colorChanged=false;
    this.active[color].push(i);
    return i;
  },
  returnInstance(i){
    i.onDeath&&i.onDeath(i);
    i.onDeath=null;i.secondColor=null;i.transitionTime=0;i.colorChanged=false;
    this._pool.push(i);
  }
};
...
const Spark = {
  ...
  add(x,y,color,angle,speed,life){
    ...
    i.speedX=Math.sin(angle)*speed;i.speedY=Math.cos(angle)*speed;
    i.life=life;
    this.active[color].push(i);
    return i;
  },
...
function createBurst(count,factory,startAngle=0,arcLength=PI_2){
  ...
  for(let i=0;i<=C_HALF;i++){
    ...
    for(let j=0;j<partsPerArc;j++){
      factory(angleInc*j+angleOffset+Math.random()*maxRandom,ringSize);
    }
  }
}
...
function createParticleArc(start,arcLen,count,rand,factory){
  ...
  if(end>start){for(let a=start;a<end;a+=d)factory(a+Math.random()*d*rand);}
  else{for(let a=start;a>end;a+=d)factory(a+Math.random()*d*rand);}
}
...
function burst(x, y, opts) {
  ...
  const starCount = opts.starCount || Math.max(6, Math.pow(opts.spreadSize/54, 2));

  let sparkFreq=0,sparkSpeed=0,sparkLife=0,sparkLifeVar=0.25;
  if(opts.glitter==='light'){sparkFreq=400;sparkSpeed=0.3;sparkLife=300;sparkLifeVar=2;}
  else if(opts.glitter==='heavy'){sparkFreq=80;sparkSpeed=0.8;sparkLife=1400;sparkLifeVar=2;}
  sparkFreq=sparkFreq/quality;

  const starFactory=(angle,speedMult)=>{
    const star=Star.add(x,y,color,angle,speedMult*speed,
      starLife+Math.random()*starLife*starLifeVar,0,-opts.spreadSize/1800);
    if(opts.pistilColor){
  ...
  if(opts.pistilColor){
    burst(x,y,{spreadSize:opts.spreadSize*0.5,starLife:starLife*0.6,color:opts.pistilColor,starCount:Math.max(4,starCount*0.4)});
  }
...
function launchShell(xFrac, heightFrac) {
  ...
  const colors = [COLOR.Red,COLOR.Green,COLOR.Blue,COLOR.Purple,COLOR.Gold,COLOR.White];
  const color = colors[Math.floor(Math.random()*colors.length)];
  const size = IS_MOBILE ? 2 : 3;
  const spreadSize = 300+size*100;

  // Cometa de subida
  const comet = Star.add(launchX,launchY,COLOR.White,Math.PI,launchVelocity,launchVelocity*400);
  comet.heavy=true;
...
overlay.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:#000;z-index:2;pointer-events:none;opacity:0.15;';
