...
function carregarEnderecoEntrega(perfil) {
    ...
    if (!perfil || !perfil.enderecos || perfil.enderecos.length === 0) {
        ...
        if (btnWrap) {
            btnWrap.dataset.tooltip =
                "Cadastre um endereço de entrega primeiro";
        }
        return;
    }
    ...
    const endereco =
        perfil.enderecos.find((e) => e.id === perfil.enderecoSelecionadoId) ||
        perfil.enderecos[0];

    if (card) {
        card.innerHTML =
            `<strong>${escHtml(endereco.rua)}, ${
                escHtml(endereco.numero)
            }</strong>` +
            (endereco.complemento
                ? `<br>${escHtml(endereco.complemento)}`
                : "") +
            `<br>${escHtml(endereco.bairro)} — ${escHtml(endereco.cidade)}/${
                escHtml(endereco.estado)
            }` +
...
function renderCupomCheckout() {
    ...
    if (_cupomCheckout) {
        ...
        if (aplicado) {
            aplicado.innerHTML = `<div>
                    <span class="cupom-aplicado-info">${
                escHtml(_cupomCheckout.codigo)
            } — ${_cupomCheckout.percentual}% OFF</span>
                    <span class="cupom-aplicado-desconto"> -T$ ${
                (_cupomCheckout.desconto || 0).toFixed(2)
            }</span>
                 </div>
        ...
        if (labelDesc) {
            labelDesc.textContent = `Desconto (${_cupomCheckout.codigo})`;
        }
        if (valorDesc) {
            valorDesc.textContent = `-T$ ${
                (_cupomCheckout.desconto || 0).toFixed(2)
            }`;
        }
    } else {
...
window.aplicarCupomCheckout = async function () {
    const codigo = (document.getElementById("checkoutCupomCodigo")?.value || "")
        .trim().toUpperCase();
    const feedback = document.getElementById("checkoutCupomFeedback");
    if (!codigo) return;
...
function renderFinanceiro(saldo, totalOriginal) {
    ...
    const total = _cupomCheckout
        ? _cupomCheckout.totalComDesconto
        : totalOriginal;
    ...
    if (saldoPos < 0) {
        ...
        if (msg) {
            msg.textContent =
                ` Você precisa de mais T$ ${falta} para concluir esta compra.`;
        }
    ...
    try {
        ...
        if (!res.ok) {
            ...
            const msg = typeof data === "string"
                ? data
                : (data.error || data.message || data.falhas?.[0]?.motivo ||
            ...
            if (msg.toLowerCase().includes("saldo")) {
                ...
                const alertaSaldoMsg = document.getElementById(
                    "alertaSaldoMsg",
                );
        ...
        const confirmacaoData = {
            comprados: (data.comprados || []).map((c) => {
                const itemLocal = _itensSelecionados.find((i) =>
                    i.id === c.livroId
                ) || {};
                let foto = null;
                try {
                ...
                if (!foto && itemLocal.isbn) {
                    foto = "https://covers.openlibrary.org/b/isbn/" +
                        itemLocal.isbn.replace(/-/g, "") + "-L.jpg";
                }
                return {
                    pedidoId: c.pedidoId,
        ...
        sessionStorage.setItem(
            "bibliotroca_confirmacao",
            JSON.stringify(confirmacaoData),
        );
...
async function buscarCepCheckout(cep) {
    const statusEl = document.getElementById("cepStatus");
    if (statusEl) {
        statusEl.className = "cep-status cep-buscando";
    ...
    try {
        ...
        if (data.erro) {
            if (statusEl) {
                statusEl.className = "cep-status cep-erro";
        ...
        if (statusEl) {
            statusEl.className = "cep-status cep-ok";
...
window.salvarEnderecoCheckout = async function () {
    ...
    const comp = (document.getElementById("fEndComplemento")?.value || "")
        .trim();
    ...
    const estado = (document.getElementById("fEndEstado")?.value || "").trim()
        .toUpperCase();
    ...
    try {
        ...
        if (btnSalvar) {
            ...
            btnSalvar.textContent = "Salvar e usar";
        }
    }
};
...
function mostrarModalExpirada() {
    ...
    if (btn) btn.disabled = true;
    setTimeout(() => {
        window.location.href = "/livros/vitrine";
    }, 5000);
}
...
function mostrarModalIndisponivel(mensagem) {
    ...
    if (msg) {
        msg.textContent = mensagem ||
            "Este produto está temporariamente reservado.";
    }
    if (modal) modal.style.display = "flex";
}
...
function iniciarCountdown(segundos) {
    ...
    const atualizar = () => {
        ...
        el.innerHTML = `⏱ Produto reservado por: <strong>${min}:${
...
async function reservarLivros(livroIds) {
    try {
        ...
        mostrarToast(
            "Não foi possível reservar o produto. Tente novamente.",
            "aviso",
        );
    }
}
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        dadosDireto =
            JSON.parse(sessionStorage.getItem(CHECKOUT_KEY + "_direto")) || [];
    } catch (_) {}
    ...
    const idsConferem = ids.every((id) => idsDireto.has(Number(id))) &&
        ids.length === idsDireto.size;
    ...
    if (dadosDireto.length > 0 && idsConferem) {
        ...
        _itensSelecionados = getCarrinho().filter((i) =>
            idsSet.has(Number(i.id))
        );
    }
    ...
    const perfil = await carregarPerfil();
    carregarEnderecoEntrega(perfil);
    const total = _itensSelecionados.reduce(
        (s, i) => s + (i.precoAprovado || 0),
        0,
    );
