    ...
    try {
        ...
        if (response.ok) {
            ...
            if (data.qrCodeBase64) {
                ...
                document.getElementById("textoCopiaECola").value =
                    data.pixCopiaECola;
            ...
            alert(
                "Erro no pagamento: " + (data.message || "Verifique os dados."),
            );
        }
    } catch (error) {
        console.error(error);
        ...
        btn.disabled = false;
        btn.innerHTML =
            '<i class="fa-solid fa-check-double"></i> Confirmar e Pagar';
    }
}
        ...
        if (res.ok) {
            ...
            tbody.innerHTML = transacoes.map((t) => `
                ...
                new Date(t.dataHora).toLocaleString("pt-BR")
                    ...
                    <td class="p-4 font-bold text-indigo-600">T$ ${
                t.valor.toFixed(2)
            }</td>
    ...
    intervaloCheck = setInterval(async () => {
        try {
            ...
            if (data.status === "APROVADO") {
                ...
                alert(
                    "🚀 Sucesso! O pagamento foi compensado e seus tokens já estão disponíveis.",
                );
...
async function simularAvisoBanco() {
    ...
    try {
        const response = await fetch(
            `/api/tokens/simular-webhook/${pagamentoId}`,
        );
        if (response.ok) {
            console.log(
                "Comando de aprovação enviado para o ID: " + pagamentoId,
            );
        }
    } catch (e) {
